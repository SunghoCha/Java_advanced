package com.stream.collectors.downstream;
    ...
    public static void main(String[] args) {
        ...
        Map<Integer, Optional<Student>> collect2 = students.stream()
                .collect(Collectors.groupingBy(
                        Student::getGrade, Collectors.reducing((s1, s2) -> s1.getScore() > s2.getScore() ? s1 : s2)
                ));
        System.out.println("collect2 = " + collect2);
        // 3 단계: 학년별로 가장 점수가 높은 학생을 구해라. maxBy 사용
        Map<Integer, Optional<Student>> collect3_1 = students.stream()
                .collect(Collectors.groupingBy(
                        Student::getGrade, Collectors.maxBy((s1, s2) -> s1.getScore() > s2.getScore() ? 1 : 0)
                ));
        System.out.println("collect3_1 = " + collect3_1);
        Map<Integer, Optional<Student>> collect3_2 = students.stream()
                .collect(Collectors.groupingBy(
                        Student::getScore, Collectors.maxBy(Comparator.comparingInt(s -> s.getScore()))
                ));
        ...
        Map<Integer, String> collect4 = students.stream()
                .collect(Collectors.groupingBy(
                        Student::getGrade, Collectors.collectingAndThen(
                                ...
                                sOptional -> sOptional.get().getName())
                ));
