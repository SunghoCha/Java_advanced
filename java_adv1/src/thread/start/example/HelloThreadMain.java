...
public class HelloThreadMain {
    ...
    public static void main(String[] args) {
        ...
        /*
            thread.start() 메서드를 통해 스레드를 실행해야만 새로운 스레드에서 run() 메서드의 코드가 실행된다는 점이 중요한 포인트.
            run()메서드의 실행 결과인 Thread-0: run()는 thread.start()로 실행된 이후의 main 스레드 흐름 중간 어느시점에 나올지 알 수 없음
            (실행 결과는 스레드의 실행 순서에 따라 약간 다를 수 있다.)

            HelloThread 스레드 객체를 생성하고 thread.start() 메서드를 호출
            thread.start() 메서드는 스레드를 실행하는 특별한 메서드이다.
            thread.start()를 호출하면 HelloThread 스레드가 run() 메서드를 실행한다.

            반드시 run() 메서드가 아닌 thread.start()를 호출해야함. 그래야 별도의 스레드에서 run() 코드가 실행됨
            (thread.start() 메서드를 호출하면 새로운 스레드가 생성되고 해당 스레드는 run() 메서드를 실행한다.
            그러나 thread.start()를 호출하지 않고 run()을 직접 호출하면 새로운 스레드가 생성되지 않고 현재 스레드에서 실행된다.)
         */
        ...
        /*
            ...
            HelloThread 스레드 객체를 생성한 다음 thread.start() 메서드를 호출하면 자바는 스레드를 위한 "별도의 스택 공간"을 할당한다.
            스레드 객체를 생성하고 반드시 thread.start()를 호출해야 스택 공간을 할당받고 스레드가 작동한다. 그전까지는 단순한 객체 덩어리에 불과.

            스레드에 이름을 주지 않으면 자바는 스레드에 Thread-0,1,2.. 같은 임의의 이름을 부여한다.
            thread.start()를 호출해 새로운 Thread-0 스레드가 사용할 전용 스택 공간이 생기며, run() 메서드의 스택 프레임을 스택에 올리면서 run()메서드를 시작한다.
         */

        /*
            여기서 핵심은 main 스레드가 run() 메서드를 실행하는게 아니라 Thread-0 스레드가 run() 메서드를 실행한다는 점이다.
            main 스레드는 단지 thread.start() 메서드를 통해 Thread-0 스레드에게 실행을 지시만 하고 바로 thread.start() 메서드를 빠져나온다.
